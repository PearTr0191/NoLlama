...
- Before diving into OpenVINO/GenAI sources, check `C:\devel\intel\openvino-map`
  — our gated, epistemically-tagged map of verified upstream behavior (sibling
  to the sparse checkouts it maps; first page: VLM backend selection /
  `requires_sdpa`). New findings go in its `PROPOSED_UPDATES.md` for human
  triage — never patch its `maps/` in passing.
